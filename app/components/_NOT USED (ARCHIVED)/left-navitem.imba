tag left-navitem
	...
	css self 
		d:hflex .modulus_active:vflex g:1sp ai:center p:1sp
	...
	def render
		<self .locked=locked .modulus_active=modulus_active>
			...
			<.modulus-info>
				<.modulus-stats>
					<h2.modulus-name>
						<slot>
						<span[ml:0.3sp]> "{name}"
					<.modulus-wordcount[ta:right]>
						<div> progress
				# let active = self.flags.contains('active')
				<progress-bar[hue:indigo $bg:gray3 @darkmode:gray7]=modulus_active progress=progress>
