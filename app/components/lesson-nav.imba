...
tag lesson-nav-item
	...
	def render
		const progress = ((state.learning_data.lesson_progress[state.course][id] / lesson.word_usage_count_sum) * 100).toFixed(2)
				...
				<[d:hflex gap:0.6sp]>
					...
					<div[d:vflex]>
						...
						<div.progress-numbers[opacity:80% fs:xs ff:monospace]>
							<span> "{progress}% ({state.learning_data.lesson_learned_usage[state.course][id]}/{lesson.word_usage_count_sum})"
